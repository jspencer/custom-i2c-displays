...
use switch_hal::{ActiveHigh, ActiveLow, IntoSwitch, OutputSwitch, Switch};

const OUR_I2C_ADDRESS: u8 = 0x34;
...
const APP: () = {
    ...
    fn init(c: init::Context) -> init::LateResources {
        ...
        let digits = [
            pa15.into_push_pull_output_with_state(&mut pins_a.crh, State::High)
                .downgrade()
                .into_active_low_switch(), // thousands
            pins_a
                .pa4
                .into_push_pull_output_with_state(&mut pins_a.crl, State::High)
                .downgrade()
                .into_active_low_switch(), // hundreds
            pins_a
                .pa10
                .into_push_pull_output_with_state(&mut pins_a.crh, State::High)
                .downgrade()
                .into_active_low_switch(), // tens
            pins_a
                .pa8
                .into_push_pull_output_with_state(&mut pins_a.crh, State::High)
                .downgrade()
        ...
        let segments = [
            pins_a
                ...
                .downgrade()
                .into_active_high_switch(), // a
            pins_a
                .pa3
                .into_push_pull_output_with_state(&mut pins_a.crl, State::Low)
                .downgrade()
                .into_active_high_switch(), // b
            pins_a
                .pa11
                .into_push_pull_output_with_state(&mut pins_a.crh, State::Low)
                .downgrade()
                .into_active_high_switch(), // c
            pins_a
                .pa6
                .into_push_pull_output_with_state(&mut pins_a.crl, State::Low)
                .downgrade()
                .into_active_high_switch(), // d
            pins_a
                .pa12
                .into_push_pull_output_with_state(&mut pins_a.crh, State::Low)
                .downgrade()
                .into_active_high_switch(), // e
            pins_a
                .pa5
                .into_push_pull_output_with_state(&mut pins_a.crl, State::Low)
                .downgrade()
                .into_active_high_switch(), // f
            pins_a
                .pa7
                .into_push_pull_output_with_state(&mut pins_a.crl, State::Low)
                .downgrade()
            ...
            pins_a
                ...
                .into_push_pull_output_with_state(&mut pins_a.crh, State::Low)
                .downgrade()
                .into_active_high_switch(), // decimal point
        ];
    ...
    fn i2c1_ev(c: i2c1_ev::Context) {
        ...
        if sr1.tx_e().bit_is_set() && sr1.btf().bit_is_clear() {
            //rprintln!("Transmit required and byte transfer not finished");
            i2c.dr.write(|w| w.dr().bits(4));
        }
...
fn output_segments(display: &mut Display, value: u8) {
    for (i, segment) in display.segments.iter_mut().enumerate() {
        if (value >> i) & 1 > 0 {
            segment.on().unwrap();
        } else {
            segment.off().unwrap();
